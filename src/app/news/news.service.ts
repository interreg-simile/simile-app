...
export class NewsService {
  ...
  constructor(private http: HttpClient, private i18n: LangService, private storageService: StorageService) { }
  ...
  async fetchAlerts() {
    ...
    const read = await this.storageService.get(this.storageKeyAlerts) as string[] || [];
  ...
  async fetchEvents() {
    ...
    const read = await this.storageService.get(this.storageKeyEvents) as string[] || [];
  ...
  async saveData(key: string, id: string) {
    const data = await this.storageService.get(key) as string[];

    if (!data) {
      return this.storageService.set(key, [id]);
    }
